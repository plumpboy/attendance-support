...
const Popup = () => {
  ...
  const handleClick = async () => {
    try {
      ...
      const timesheetOverview = await fetchLogData(
      ...
      const workLogsList = timesheetOverview?.timesheetOverview.filter(
        (log) => {
          return (
            !log.isWeekend &&
            !log.isHoliday &&
            ...
            (!log.workLogs ||
              log.workLogs?.totalHours < log.allocated?.totalHours)
          );
        }
      );
      ...
      if (workLogsList.length > 0) {
        for (let i = 0; i < workLogsList.length; i++) {
          ...
          for (let j = 0; j < allowcatedData?.length; j++) {
            const logHour = workLogsList?.[i].workLogs
              ? Object.values(workLogsList?.[i]?.workLogs?.detail).find(
                  (data) => data.code === allowcatedData[j].code
                )?.hours
              : 0;
            ...
            let workLogsData = [
              {
                ...
                description: null,
                workHours: logHour
                  ? allowcatedData[j]?.hours - logHour
                  : allowcatedData[j]?.hours,
                typeOfWork: workType,
                projectId: res.find(
                  (project) => project.code === allowcatedData[j].code
                )?.id,
              },
            ];
    ...
    <div className="App">
      <header className="App-header">
        <div class="title">
          ...
          <p class="text-title">Zoho Attendance Support</p>
        </div>
        {isOnSRAPage ? (
          <div class="container">
          ...
          <div class="go-to-sra">
            <button className="log-button" type="button" onClick={goToSRA}>
              Go to SRA
            </button>
            <br />
